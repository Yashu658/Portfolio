...
const UpdateProfile = ({ showModal, setShowModal }) => {
  ...
  useEffect(() => {
    if (showModal) {
      api.get("/user/profile").then(res => {
        ...
        setUser({
          fullName: employee.personalDetails?.name || "",
          department: employee.jobDetail?.department || "",
          position: employee.jobDetail?.designation || "",
          email: employee.personalDetails?.emailID || "",
          phone: employee.personalDetails?.mobile || "",
          dob: employee.personalDetails?.dob || "",
          gender: employee.personalDetails?.gender || "",
          profilePic: employee.personalDetails?.image || "",
          qualification: employee.educationalDetails?.[0]?.degree || "",
          address: employee.personalDetails?.correspondenceAddress || "",
        });
      });
    }
  }, [showModal]);
